...
def __get_kedro_context__():
    ...
    from give_me_some_credit.run import __kedro_context__
...
def run(tag, env, parallel, runner):
    """Run the pipeline."""
    from give_me_some_credit.run import main
...
def build_docs():
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=give_me_some_credit"]
    )
